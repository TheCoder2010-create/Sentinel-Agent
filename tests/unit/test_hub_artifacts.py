...
def test_artifact_collection_title_uses_session_date_and_id():
    assert (
        ...
        == "sentinel-ai-artifacts-2026-05-05-session-123"
    )
...
def test_artifact_collection_title_uses_short_uuid_fragment():
    ...
    assert title == "sentinel-ai-artifacts-2026-05-05-fadcbc77"
    assert len(title) < 60
...
def test_artifact_collection_title_still_truncates_long_non_uuid_ids():
    ...
    assert title.startswith("sentinel-ai-artifacts-2026-05-05-custom-session-id")
    assert len(title) < 60
...
"""
    ...
    assert f"- {SENTINEL_AI_TAG}" in updated
...
def test_register_hub_artifact_creates_private_collection_and_adds_item_once(
    ...
    class FakeApi:
        ...
        def create_collection(self, **kwargs):
            ...
            return SimpleNamespace(slug="alice/sentinel-ai-artifacts")
    ...
    assert b"sentinel-ai" in api.uploads[0]["path_or_fileobj"]
...
def test_sitecustomize_bootstrap_is_valid_python():
    ...
    assert "sentinel-ai-artifacts-2026-05-05-session-123" in code
...
def test_sitecustomize_bootstrap_reuses_existing_collection_slug():
    ...
    setattr(
        ...
        "alice/sentinel-ai-artifacts-2026-05-05-session-123",
    )
    ...
    compile(code, "sitecustomize.py", "exec")
    assert (
        "collection_slug = 'alice/sentinel-ai-artifacts-2026-05-05-session-123'" in code
    )
...
def test_sitecustomize_caches_lazy_collection_slug_across_bootstraps(
    ...
    collection_slug = "alice/sentinel-ai-artifacts-2026-05-05-session-123"
    uploads = []
    downloads = []
    collection_creates = []
    collection_items = []
    ...
    monkeypatch.setenv("SENTINEL_AI_ARTIFACT_COLLECTION_CACHE", str(cache_path))
    code = build_hub_artifact_sitecustomize(_session())
...
def test_sitecustomize_skips_sandbox_space_registration(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv(
        "SENTINEL_AI_ARTIFACT_COLLECTION_CACHE",
