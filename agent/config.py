...
def apply_slack_user_defaults(raw_config: dict[str, Any]) -> dict[str, Any]:
    ...
    if not _env_bool("SENTINEL_AI_SLACK_NOTIFICATIONS", True):
        return raw_config
    ...
    destination_name = (
        os.environ.get("SENTINEL_AI_SLACK_DESTINATION") or SLACK_DEFAULT_DESTINATION
    ).strip()
    ...
    if destination_name not in destinations:
        destinations[destination_name] = {
            ...
            "token": token,
            "channel": channel,
            "allow_agent_tool": _env_bool("SENTINEL_AI_SLACK_ALLOW_AGENT_TOOL", True),
            "allow_auto_events": _env_bool("SENTINEL_AI_SLACK_ALLOW_AUTO_EVENTS", True),
        }

    auto_events = _env_list("SENTINEL_AI_SLACK_AUTO_EVENTS")
