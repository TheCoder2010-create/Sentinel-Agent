...
from agent.utils.boot_timing import settle_curve, blue_from_white
...
def run_particle_logo(console: Console, hold_seconds: float = 1.5) -> None:
    ...
    step = max(1, len(all_targets) // 1200)
    ...
    with Live(console=console, refresh_per_second=fps, transient=True) as live:
        for frame in range(total_frames):
            ...
            if frame < converge_frames:
                ...
                # Color transitions from white to blue
                r, g, b = blue_from_white(progress)
            else:
    ...
    for line in canvas.render():
        for ch in line:
            if ch == chr(0x2800):
                ...
                final.append(ch, style="rgb(80,160,255)")
        final.append("\n")
