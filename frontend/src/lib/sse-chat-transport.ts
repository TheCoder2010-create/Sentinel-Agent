...
export interface SideChannelCallbacks {
  ...
  onUsageEvent: (eventType: 'llm_call' | 'sandbox_destroy', data: Record<string, unknown>) => void;
        ...
        case 'tool_state_change': {
          ...
          if (state === 'billing_required') {
            ...
            useAgentStore.getState().setJobsUpgradeRequired({
              ...
              message: namespace
                ? `PlatformOps Jobs need credits on the "${namespace}" namespace. Job credits are separate from HF Pro membership; add credits, then re-run the same job.`
                : 'PlatformOps Jobs need namespace credits, which are separate from HF Pro membership. Add credits, then re-run the same job.',
            });
          }
          break;
        }
