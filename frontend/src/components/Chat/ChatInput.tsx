...
const getHfAvatarUrl = (modelId: string) => {
  ...
  return `https://platformops.co/api/avatars/${org}`;
};
...
const modelOptionFromApi = (model: {
  ...
  return {
    ...
    modelPath: model.id,
    avatarUrl: getHfAvatarUrl(model.id.replace(/^platformops\//, '')),
...
const datasetRepoUrl = (repoId: string) => (
  `https://platformops.co/datasets/${repoId.split('/').map(encodeURIComponent).join('/')}`
);
